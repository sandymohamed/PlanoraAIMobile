...
export const PaywallScreen: React.FC = () => {
  ...
  const rowDir = {
    flexDirection: isArabic ? ("row-reverse" as const) : ("row" as const),
  };
  ...
  const remainingLabel = isPremium
    ...
    : aiPlansLimit != null
      ? t("paywall.usage.remainingWithLimit", {
          remaining,
          limit: aiPlansLimit,
        })
  ...
  const handleJoin = async () => {
    ...
    if (!EMAIL_RE.test(trimmed)) {
      showError(
        t("paywall.errors.invalidEmailTitle"),
        t("paywall.errors.invalidEmailMessage"),
      );
      return;
    }
    setSubmitting(true);
    ...
    try {
      ...
      showError(
        t("paywall.errors.waitlistTitle"),
        t("paywall.errors.waitlistMessage"),
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      >
        ...
        <Text style={[styles.heroTitle, textDir]}>
          {t("paywall.heroTitle")}
        </Text>
      ...
      <Text style={[styles.sectionTitle, textDir]}>
        {t("paywall.sectionTitle")}
      </Text>
        ...
        <Card key={key} style={styles.featureCard}>
          <View style={[styles.featureRow, rowDir]}>
            ...
            <View style={styles.featureTextWrap}>
              <Text style={[styles.featureTitle, textDir]}>
                {t(`paywall.features.${key}.title`)}
              </Text>
              <Text style={[styles.featureSubtitle, textDir]}>
                {t(`paywall.features.${key}.subtitle`)}
              </Text>
            </View>
          </View>
        </Card>
      ...
      <Card elevated style={styles.waitlistCard}>
        {joined ? (
          <View style={styles.joinedWrap}>
            ...
            <Text style={[styles.joinedTitle, textDir]}>
              {t("paywall.waitlist.joinedTitle")}
            </Text>
            <Text style={styles.joinedSub}>
              {t("paywall.waitlist.joinedSub")}
            </Text>
          </View>
        ) : (
          <>
            <Text style={[styles.waitlistTitle, textDir]}>
              {t("paywall.waitlist.title")}
            </Text>
            <Text style={[styles.waitlistSub, textDir]}>
              {t("paywall.waitlist.subtitle")}
            </Text>
            <TextInput
            ...
            <Button
              label={
                submitting
                  ? t("paywall.waitlist.joining")
                  : t("paywall.waitlist.notifyMe")
              }
              onPress={handleJoin}
              disabled={submitting}
            />
