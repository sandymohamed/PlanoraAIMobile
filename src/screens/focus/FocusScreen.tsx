...
export const FocusScreen: React.FC = () => {
  ...
  const [secondsLeft, setSecondsLeft] = useState(
    FOCUS_MODES.pomodoro.minutes * 60,
  );
  ...
  const handleComplete = useCallback(async () => {
    ...
    const updated = await recordCompletedSession(totalSecRef.current).catch(
      () => null,
    );
  ...
  useFocusEffect(
    useCallback(() => {
      let cancelled = false;
      (async () => {
        const [session, todayStats] = await Promise.all([
          loadSession(),
          loadStats(),
        ]);
        if (cancelled) return;
  ...
  const onStart = async () => {
    track(AnalyticsEvents.FOCUS_SESSION_STARTED, {
      mode,
          ...
          {status === "idle" && (
            ...
            >
              ...
              <Text style={styles.stepLabel}>
                {Math.round(secondsLeft / 60)} {t("focusScreen.min")}
              </Text>
              <TouchableOpacity
                style={styles.stepBtn}
                onPress={() => adjustMinutes(5)}
        ...
        {status === "running" ? (
          ...
          >
            ...
            <Text style={styles.startText}>
              {status === "paused"
                ? t("focusScreen.resume")
                : t("focusScreen.startSession")}
            </Text>
          </TouchableOpacity>
        )}
        {status !== "idle" && (
          ...
          <View style={styles.modalCard}>
            ...
            >
              {t("focusScreen.sessionCompleteMessage", {
                mode: completedMode
                  ? t(`focusScreen.modes.${completedMode}.label`)
                  : t("focusScreen.title"),
              })}
            </Text>
            <TouchableOpacity
