...
class AlarmApiService {
  // Alarm methods
  async getAlarms(page = 1, limit = 20, enabled?: boolean): Promise<PaginatedResponse<Alarm>> {
    try {
      const params: any = { page, limit };
      if (enabled !== undefined) {
        params.enabled = enabled;
      }
      const response = await apiClient.get<PaginatedResponse<Alarm>>('/alarms', { params });
      return response;
    } catch (error) {
  ...
  async createAlarm(data: CreateAlarmData): Promise<Alarm> {
    try {
      const response = await apiClient.post<ApiResponse<Alarm>>('/alarms', data);
      if (!response.success) {
        throw new Error(response.error || 'Failed to create alarm');
      }
      return response.data;
    } catch (error) {
      logger.error('Create alarm error:', error);
      throw error;
    }
  }
  ...
  async updateAlarm(id: string, data: UpdateAlarmData): Promise<Alarm> {
    try {
      const response = await apiClient.put<ApiResponse<Alarm>>(`/alarms/${id}`, data);
      if (!response.success) {
        throw new Error(response.error || 'Failed to update alarm');
      }
      return response.data;
    } catch (error) {
      logger.error('Update alarm error:', error);
      throw error;
    }
  }
  ...
  async snoozeAlarm(id: string, duration?: number): Promise<void> {
    try {
      const response = await apiClient.post<ApiResponse<void>>(`/alarms/${id}/snooze`, { duration });
      if (!response.success) {
        throw new Error(response.error || 'Failed to snooze alarm');
      }
    } catch (error) {
      logger.error('Snooze alarm error:', error);
      throw error;
    }
  }
  ...
  async dismissAlarm(id: string): Promise<void> {
    try {
      const response = await apiClient.post<ApiResponse<void>>(`/alarms/${id}/dismiss`);
      if (!response.success) {
        throw new Error(response.error || 'Failed to dismiss alarm');
      }
    } catch (error) {
      logger.error('Dismiss alarm error:', error);
      throw error;
    }
  }
  ...
  async getTimers(page = 1, limit = 20): Promise<PaginatedResponse<Timer>> {
    try {
      ...
      const response = await apiClient.get<PaginatedResponse<Timer>>('/timers', { 
        params: { page, limit } 
      });
      return response;
    } catch (error) {
      logger.error('Get timers error:', error);
      throw error;
    }
  }
  ...
  async createTimer(data: CreateTimerData): Promise<Timer> {
    try {
      const response = await apiClient.post<ApiResponse<Timer>>('/timers', data);
      if (!response.success) {
  ...
  async updateTimer(id: string, data: UpdateTimerData): Promise<Timer> {
    try {
      const response = await apiClient.put<ApiResponse<Timer>>(`/timers/${id}`, data);
      if (!response.success) {
        throw new Error(response.error || 'Failed to update timer');
      }
      return response.data;
    } catch (error) {
      logger.error('Update timer error:', error);
      throw error;
    }
  }
  ...
  async deleteTimer(id: string): Promise<void> {
    try {
      const response = await apiClient.delete<ApiResponse<void>>(`/timers/${id}`);
      if (!response.success) {
        throw new Error(response.error || 'Failed to delete timer');
      }
    } catch (error) {
      logger.error('Delete timer error:', error);
      throw error;
    }
  }
  ...
  async startTimer(id: string): Promise<Timer> {
    try {
      const response = await apiClient.post<ApiResponse<Timer>>(`/timers/${id}/start`);
      if (!response.success) {
  ...
  async pauseTimer(id: string): Promise<Timer> {
    try {
      const response = await apiClient.post<ApiResponse<Timer>>(`/timers/${id}/pause`);
      if (!response.success) {
  ...
  async stopTimer(id: string): Promise<Timer> {
    try {
      const response = await apiClient.post<ApiResponse<Timer>>(`/timers/${id}/stop`);
      if (!response.success) {
  ...
  async resetTimer(id: string): Promise<Timer> {
    try {
      const response = await apiClient.post<ApiResponse<Timer>>(`/timers/${id}/reset`);
      if (!response.success) {
