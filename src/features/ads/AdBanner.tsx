...
const HIDDEN_AD_CHANNELS = new Set(['development', 'internal', 'internal-testing', 'closed', 'closed-testing']);
...
function canRenderProductionBanner(isPremium: boolean, adsRemoved: boolean) {
  ...
  if (!isFeatureEnabled('ENABLE_ADS') || !isFeatureEnabled('ENABLE_BANNER_ADS')) return false;
...
export const AdBanner: React.FC<AdBannerProps> = ({ style }) => {
  ...
  return (
    <View style={[styles.slot, style]} pointerEvents="none" importantForAccessibility="no-hide-descendants">
      {shouldRenderBanner ? (
